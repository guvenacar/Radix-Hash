#model/radix_hash.py

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
772-bitlik QTHash algoritmasının çekirdek fonksiyonları.
Bu modül, NIST test betiği tarafından içe aktarılmak üzere tasarlanmıştır.
"""
import sys
import os
import time

# M değeri. Bit uzunluğu 772'ye en yakın olan 3'ün kuvveti.
# 3**486 sayısı yaklaşık 770 bit uzunluğundadır ve 772 bitlik hedefe en yakın değeri sağlar.
M = 3 ** 486

def text_to_bits(s: str) -> str:
    """Metni UTF-8'e çevirir ve bit string olarak döndürür."""
    return ''.join(f"{b:08b}" for b in s.encode("utf-8"))

def pad_bits(bits: str, size=772) -> str:
    """Bit string'ini 772'nin katı olacak şekilde doldurur."""
    if len(bits) % size == 0:
        return bits
    return bits + "0" * (size - len(bits) % size)

def chunks(bits: str, size=772):
    """Bit string'ini 772-bitlik parçalara ayırır."""
    return [bits[i:i+size] for i in range(0, len(bits), size)]

def bits_to_base3_int(bits: str) -> int:
    """Bit string'ini 3 tabanında bir tamsayıya dönüştürür."""
    n = 0
    for bit in bits:
        n = n * 3 + (1 if bit == "0" else 2)
    return n

def normalize_bits(bits: str, target_length=771) -> str:
    """Bit string'ini 772 bit'e normalize eder."""
    if len(bits) == target_length:
        return bits + "1"
    return bits

def _hash_block_internal(bits: str) -> str:
    """
    Tek bir 772-bitlik bloğu işler ve heksadesimal bir string döndürür.
    Bu fonksiyona dışarıdan doğrudan çağrı yapılmamalıdır.
    """
    n = bits_to_base3_int(bits)
    hexstr = format(n, "x")
    vals = [int(ch, 16) + 2 for ch in hexstr]

    total = 2
    term = 0
    i = 0
    while i < len(vals) - 1:
        a, b = vals[i], vals[i+1]
        term = (term + pow(a, b, M)) % M
        total = (total * term) % M
        i += 2

    if i < len(vals):  # Tek eleman kalırsa
        last = vals[i]
        term = pow(last, 3, M)
        total = (total + term) % M

    # Normalize çıkışta
    if total.bit_length() < 771:
        total = 2 * M - total
    elif total.bit_length() == 771:
        total = total + 1

    return format(total, "x")

from utils.balance_transforms import xor_not_reverse_dynamic_count

def process_block(input_data: str) -> str:
    """
    Ana test betiği tarafından çağrılan fonksiyon.
    Metin girdisi alır, hash'i hesaplar ve 772-bitlik bir string döndürür.
    Çok bloklu girdileri XOR ile birleştirir.
    Scrambling: XOR + NOT + reverse + dynamic count interleave.
    """
    BITS_PER_BLOCK = 772
    
    # Girdiyi bitlere çevir, doldur ve parçalara ayır
    bits = pad_bits(text_to_bits(input_data), BITS_PER_BLOCK)
    bit_chunks = chunks(bits, BITS_PER_BLOCK)
    
    final_hash_int = 0
    
    for chunk in bit_chunks:
        # 1. Normalizasyon
        normalized_chunk = normalize_bits(chunk)
        
        # 2. Scrambling uygulama
        scrambled_chunk = xor_not_reverse_dynamic_count(normalized_chunk)
        
        # 3. _hash_block_internal ile hashle
        hex_hash = _hash_block_internal(scrambled_chunk)
        
        # 4. XOR ile birleştir
        final_hash_int ^= int(hex_hash, 16)

    # Sonucu 772-bitlik bir bit string'ine dönüştür
    final_hash_int &= ((1 << BITS_PER_BLOCK) - 1)
    return bin(final_hash_int)[2:].zfill(BITS_PER_BLOCK)

